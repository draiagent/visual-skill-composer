...
constraint. Eight project types compile today: `academic-presentation`, `website`, `video`,
`dashboard`, `report`, `infographic-card`, `comic`, and `social`. Only `brand-kit` has
no Standard VAC yet, and is refused rather than guessed. See
